...
The save never persists. This reproduces for **any** custom entity (verified on `investment:product` and `investment:campaign`); it is independent of which field the user changed. Creating a *new* record usually works (no `id`/`updated_at` present yet), so the bug is specific to the **edit** path.
...
```json
{ "entityId": "investment:product", "recordId": "be5f9bbe-…",
  "values": { "id": "be5f9bbe-…", "name": "Projekt Magnolia", "issuer": "bb9a5c4f-…",
              "goal_amount": 10000000, /* …declared fields… */,
...
- A `PUT` with `values: { name, issuer }` (no system keys) → **200**, value persisted.
...
Manual (done): on `investment:product` `be5f9bbe-…`, selected the relation `issuer`, Save → redirected to the list, issuer saved.
