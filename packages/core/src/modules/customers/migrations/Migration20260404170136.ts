...
export class Migration20260404170136 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`alter table "req_request_comments" drop constraint "req_request_comments_request_id_fkey";`);

    this.addSql(`alter table "req_request_items" drop constraint "req_request_items_request_id_fkey";`);
    ...
    this.addSql(`create index "customer_leads_outcome_idx" on "customer_leads" ("organization_id", "tenant_id", "outcome", "created_at");`);
    this.addSql(`create index "customer_leads_pipeline_stage_idx" on "customer_leads" ("organization_id", "tenant_id", "pipeline_id", "stage_id", "created_at");`);

    this.addSql(`create table "customer_lead_history" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "lead_id" uuid not null, "event_type" text not null, "actor_user_id" uuid null, "snapshot_before" jsonb null, "snapshot_after" jsonb null, "created_at" timestamptz not null, constraint "customer_lead_history_pkey" primary key ("id"));`);
    this.addSql(`create index "customer_lead_history_org_tenant_idx" on "customer_lead_history" ("organization_id", "tenant_id");`);
    this.addSql(`create index "customer_lead_history_lead_idx" on "customer_lead_history" ("lead_id", "created_at");`);
    ...
    this.addSql(`create index "customer_lead_lost_reasons_org_tenant_idx" on "customer_lead_lost_reasons" ("organization_id", "tenant_id");`);
    ...
    this.addSql(`create table "customer_lead_pipeline_stages" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "pipeline_id" uuid not null, "name" text not null, "code" text not null, "position" int not null, "kind" text not null, "is_active" boolean not null default true, "created_at" timestamptz not null, "updated_at" timestamptz not null, constraint "customer_lead_pipeline_stages_pkey" primary key ("id"));`);
    ...
    this.addSql(`create index "customer_lead_pipeline_stages_pipeline_idx" on "customer_lead_pipeline_stages" ("pipeline_id", "position");`);
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`create table "mikro_orm_migrations_requests" ("id" serial primary key, "name" varchar(255) null, "executed_at" timestamptz(6) null default CURRENT_TIMESTAMP);`);
    ...
    this.addSql(`create table "req_request_comments" ("id" uuid not null default gen_random_uuid(), "request_id" uuid not null, "author_id" uuid not null, "author_name" text not null, "content" text not null, "created_at" timestamptz(6) not null, constraint "req_request_comments_pkey" primary key ("id"));`);

    this.addSql(`create table "req_request_items" ("id" uuid not null default gen_random_uuid(), "request_id" uuid not null, "sku" text null, "name" text not null, "quantity" numeric not null, "status" text not null default 'DO_ZAMOWIENIA', "delivery_date" timestamptz(6) null, "supplier_order_number" text null, "note" text null, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, constraint "req_request_items_pkey" primary key ("id"));`);

    this.addSql(`create table "req_requests" ("id" uuid not null default gen_random_uuid(), "organization_id" uuid not null, "tenant_id" uuid not null, "request_number" text not null, "nip" text not null, "company_name" text not null, "content" text null, "customer_order_number" text null, "status" text not null default 'NIEPRZYDZIELONE', "salesperson_id" uuid null, "salesperson_name" text null, "buyer_id" uuid null, "buyer_name" text null, "unread_count" int4 not null default 0, "created_at" timestamptz(6) not null, "updated_at" timestamptz(6) not null, constraint "req_requests_pkey" primary key ("id"));`);
    ...
    this.addSql(`alter table "req_request_comments" add constraint "req_request_comments_request_id_fkey" foreign key ("request_id") references "req_requests" ("id") on update cascade on delete cascade;`);

    this.addSql(`alter table "req_request_items" add constraint "req_request_items_request_id_fkey" foreign key ("request_id") references "req_requests" ("id") on update cascade on delete cascade;`);
