...
export async function POST(req: Request) {
  return executeLeadWorkflowCommand(req, 'customers.leads.unlink-deal', customerLeadUnlinkTargetSchema)
}

export const openApi: OpenApiRouteDoc = {
  tag: 'Customers',
  summary: 'Unlink a deal from a lead',
  methods: {
    POST: {
      summary: 'Unlink deal from lead',
      requestBody: { contentType: 'application/json', schema: customerLeadUnlinkTargetSchema },
      responses: [{ status: 200, description: 'Deal unlinked', schema: z.object({ ok: z.boolean().optional() }) }],
    },
  },
}
